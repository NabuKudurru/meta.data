...
The meta.data() R package enables researchers to create, write out, edit, and reapply meta.data templates to the variables in their data sets.

These meta.data templates are for specific variables and sets of variables that are commonly used like demographic variables or standard social science scales. These templates can be stored online, pulled into a programming environment and applied onto variables, even automatically. 
...
```
Technically we do not need to create a new version of the variable, but we do need to assign the output of the function to something at the end. The function will automatically take the variable name and enter it as a variable label, which is important for combining sets of templates. We can enter any character vector we like into each of the sections, including other data sets which will be stored as meta.attributes of the variable e.g., particular other ISO standards in JSON variables. 
